...
const Table = ({ 
  ...
  const renderActionButtons = (code: string) => (
    <div className="flex items-center space-x-3.5">
      ...
      <button onClick={() => handleDeleteClick(code)} className="hover:text-primary">
        <svg
          className="fill-current"
          width="18"
          height="18"
  ...
  const renderCell = (item: any, column: string, colIndex: number) => {
    ...
    } else if (["code", "kode meja", "kode_meja"].includes(columnLower)) {
      return (
        <td key={colIndex} className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
          <p className="text-black dark:text-white">{item.code || '-'}</p>
        </td>
      );
    } else if (["capacity", "kapasitas"].includes(columnLower)) {
      return (
        <td key={colIndex} className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
          <p className="text-black dark:text-white">{item.capacity || '-'}</p>
        </td>
      );
    } else {
      // For any other column, try to find the property in the item
      const propName = column.charAt(0).toLowerCase() + column.slice(1);
      return (
        <td key={colIndex} className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
          <p className="text-black dark:text-white">
            {item[columnLower] || item[propName] || item[column] || '-'}
          </p>
        </td>
      );
    }
